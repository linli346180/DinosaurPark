...
export class Simulator {
    private agentId: number = 0;        //用于生成唯一的智能体 ID。
    public agentIdLst: number[] = [];   //存储所有智能体的 ID。
    public obstacles: Obstacle[] = [];     //存储所有障碍物的引用。
    public kdTree: KdTree = new KdTree();  //用于加速搜索邻居的 KD 树。

    private aid2agent: { [key: string]: Agent } = Object.create(null);  //存储所有代理的引用。
    private defaultAgent: Agent;        //存储默认代理的引用。
    private time: number = 0.0;         //模拟器的全局时间。
    ...
    /** 获取代理 */
    getAgent(idx: number): Agent {
        return this.aid2agent[this.agentIdLst[idx]];
    }

    getAgentByAid(aid: number): Agent {
