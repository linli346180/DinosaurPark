...
export class KdTree {
...

            {
                ...
                for (let j = 0; j < obstacles.length; ++j) {
                    ...
                    if (j1LeftOfI >= -RVOMath.RVO_EPSILON && j2LeftOfI >= -RVOMath.RVO_EPSILON) {
                        ...
                        /* Split obstacle j. */
                        tmpVec1.setVec(obstacleI2.point).minus(obstacleI1.point, false);

                        let t = RVOMath.det(tmpVec1, tmpVec2.setVec(obstacleJ1.point).minus(obstacleI1.point, false)) /
                            RVOMath.det(tmpVec1, tmpVec2.setVec(obstacleJ1.point).minus(obstacleJ2.point, false));

                        let splitpoint = obstacleJ1.point.plus((tmpVec1.setVec(obstacleJ2.point).minus(obstacleJ1.point, false)).scale(t, false));
    ...
    queryObstacleTreeRecursive(agent: Agent, rangeSq: number, node: ObstacleTreeNode) {
        if (node == null) {
            ...
            let distSqLine = RVOMath.sqr(agentLeftOfLine) / RVOMath.absSq(Vector2.minus(obstacle2.point, obstacle1.point));
    ...
    queryVisibilityRecursive(q1: Vector2, q2: Vector2, radius: number, node: ObstacleTreeNode): any {
        if (node == null) {
            return true;
        }
        else {
            ...
            let invLengthI = 1.0 / RVOMath.absSq(Vector2.minus(obstacle2.point, obstacle1.point));
            ...
            if (q1LeftOfI >= 0 && q2LeftOfI >= 0) {
                ...
                let invLengthQ = 1.0 / RVOMath.absSq(Vector2.minus(q2, q1));
