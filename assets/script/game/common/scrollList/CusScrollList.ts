import { _decorator, Node, Prefab, instantiate, ScrollView, UITransform, Vec3, log, Vec2 } from 'cc';
...
@ccclass('CusScrollList')
export class CusScrollList extends ScrollView {
    /**item子节点预制体*/
    ...
    start() {
        super.start();
        this.node.on(ScrollView.EventType.SCROLLING, this.scrollCheck, this);
    }

    onDestroy() {
        super.onDestroy();
        if (this.node) {
            this.node.off(ScrollView.EventType.SCROLLING, this.scrollCheck, this);
        }
    }
