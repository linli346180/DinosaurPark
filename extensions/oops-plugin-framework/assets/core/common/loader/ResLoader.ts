...
oops.res.load("spine_path", sp.SkeletonData, (err: Error | null, sd: sp.SkeletonData) => {
    ...
    /** 预加载单个资源 */
    preloadAsset<T extends Asset>(bundleName: string, paths: string | string[], type: AssetType<T> | null): Promise<T> {
    ...
    /** 预加载目录 */
    // preLoadDir<T extends Asset>(bundleName: string, dirPath: string, type: AssetType<T> | null): Promise<T> {
    //     return new Promise((resolve, reject) => {
    //         if (bundleName == null) bundleName = this.defaultBundleName;
    //         var bundle: AssetManager.Bundle = assetManager.getBundle(bundleName)!;
    //         bundle.preloadDir(dirPath, type, (err) => {
    //             if (err) {
    //                 console.error(`Failed to preload directory: ${dirPath}`, err);
    //                 reject(err);
    //             } else {
    //                 console.log(`Directory ${dirPath} preloaded successfully`);
    //                 resolve(null as any); // 预加载不返回资源，load 时才会真正获取
    //             }
    //         });
    //     });
...
oops.res.loadDir("game", onProgressCallback, onCompleteCallback);
    ...
    private loadByBundleAndArgs<T extends Asset>(bundle: AssetManager.Bundle, args: ILoadResArgs<T>): void {
        ...
        // console.error("loadByBundleAndArgs", args.paths);
