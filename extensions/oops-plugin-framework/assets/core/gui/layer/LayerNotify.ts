...
export class LayerNotify extends Node {
     ...
     */
    async toast(content: string, useI18n: boolean): Promise<void> {
        try {
            if (this.notify == null) {
                this.notify = await ViewUtil.createPrefabNodeAsync(ToastPrefabPath);
                if(this.notify == null) {
                    console.error("创建UI失败"+ ToastPrefabPath) 
                    return;
                }
                this.notifyItem = find("item", this.notify)!;
                this.notifyItem.parent = null;
                // this.notify = ViewUtil.createPrefabNodeAsync(ToastPrefabPath);
            }
